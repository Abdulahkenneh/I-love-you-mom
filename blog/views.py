...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            new_user = Person.objects.create_user(username=username, password=password, firstname=firstname,
                                                  lastname=lastname)
            new_user.save()
...
def mytopic_detail(request, topic_id):
    ...
    UserTopicProgress.objects.get_or_create(user=request.user, topic=mytopic,
                                            defaults={'date_accessed': timezone.now()})
